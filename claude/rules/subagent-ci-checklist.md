...
- Swagger drift: If you add/modify HTTP handlers with swagger annotations, run `swag init -g cmd/subnetree/main.go -o api/swagger --parseDependency --parseInternal`
...
Backend:
1. `go build ./...` && `go test ./...`
2. If HTTP handlers added: `swag init -g cmd/subnetree/main.go -o api/swagger --parseDependency --parseInternal`
3. Watch for: gosec G101, gocritic unnamedResult (`:=` -> `=` + remove `var` after named returns), gocritic appendCombine, bodyclose

Frontend:
1. `cd web && npx tsc --noEmit` && `npx eslint src/<your-files>`
2. Recharts Tooltip: use `Partial<TooltipContentProps<...>>` with JSX element form, or render function
3. JSX `unknown` type: use `!= null` check, not bare `&&`
4. Verify all imports are used (ESLint catches what tsc misses)
```
